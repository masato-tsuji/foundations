...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const prefElms = document.querySelectorAll(".pref");
    
    // 各県クリックイベント実装
    prefElms.forEach(elm => {
        elm.addEventListener("click", (event) => {
            // console.log(prefInfo.id[elm.id].prefName);
            console.log(findPref(elm.id).prefName);
...
function csv2json() {
    const csvFilePath = './img/pref_info.csv'
    const jsonFilePath = './img/pref_info.json'
